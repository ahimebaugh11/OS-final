...
class TestableWebbable(SetupMagic):
    ...
    def _advance(self):
        ...
        raise UrlRedirectException(
            ''.join([self.session.config.sys.http_path, url, '?%s' % qs if qs else '']))
...
#        support it: https://bugzilla.mozilla.org/show_bug.cgi?id=1166625
    ...
    def _get_email_settings(self, email):
        ...
        if not settings and self.deadline > time.time():
            ...
            settings = self._get_domain_autoconfig(
                email, domain, None, ssl=True)
...
class SetupWelcome(TestableWebbable):
    ...
    def configure_language(self, session, config, language, save=True):
        try:
            def i18n(): return ActivateTranslation(session, config, language)
...
class ResetTags(self, session,
                save_and_update_workers=True,
                want_daemons=True):
    session.ui.notify(_('Disabling lockdown'))
    security.DISABLE_LOCKDOWN = True
    SYNOPSIS = (None, 'tagreset', 'ResetTags', None)
    HTTP_CALLABLE = ('POST')
    HTTP_AUTH_REQUIRED = True
    ...
    def delete_tags(self):
        ...
        return self._success(_('Deleted all tags') % len(result),
                             {'removed': result})

    def reset_tags(self, session,
                   save_and_update_workers=True,
                   want_daemons=True):
        session.ui.notify(_('Disabling lockdown'))
        security.DISABLE_LOCKDOWN = True

        # rereate standard tags and filters
        created = []
        for t, tag_settings in self.TAGS.iteritems():
            tag_settings = copy.copy(tag_settings)
        ...
        return self._success('Reset Default Tags')
